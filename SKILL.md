...
runtime:
  foundry:
    ...
    install_termux: "DO NOT use foundryup on Termux. The alpine/arm64 static build has a TLS segment with 8-byte alignment that Bionic's loader rejects ('segment is underaligned: alignment is 8, needs to be at least 64 for ARM64 Bionic'). Instead, download foundry_1.7.1-1_aarch64.deb from packages.termux.dev, extract with dpkg-deb, and copy data/data/com.termux/files/usr/bin/{cast,forge,anvil,chisel} to both $PREFIX/bin/ and $HOME/.foundry/bin/. The install.sh does this automatically."
    ...
    solc_install_termux: "Run ./install.sh. Manual fallback: download solidity_0.8.35_aarch64.deb from packages.termux.dev, extract with dpkg-deb (preferred) or ar+tar with xzcat/xz/python3 fallbacks if Termux's tar can't handle xz, copy data/data/com.termux/files/usr/bin/solc to $PREFIX/bin/solc. Bionic rejects the e_type=2 static linux-arm64 build, so this path is mandatory on Termux."
    foundry_install_termux: "curl -fsSL 'https://packages.termux.dev/apt/termux-main/pool/main/f/foundry/foundry_1.7.1-1_aarch64.deb' -o /tmp/foundry.deb && dpkg-deb -x /tmp/foundry.deb /tmp/foundry-extract && cp /tmp/foundry-extract/data/data/com.termux/files/usr/bin/{cast,forge,anvil,chisel} $PREFIX/bin/ && cp /tmp/foundry-extract/data/data/com.termux/files/usr/bin/{cast,forge,anvil,chisel} $HOME/.foundry/bin/ && chmod +x $PREFIX/bin/{cast,forge,anvil,chisel}"
test:
